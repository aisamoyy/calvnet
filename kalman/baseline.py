...
def curriculum_pde():
	curriculum=[]
	for i in range(10):
		curriculum.append(lambda t,out:kalman_pde(t,out,10/10))
	return curriculum
...
if __name__ == '__main__':
	...
	for epoch in range(iterations):
		...
		net_end_x = net_x(t_end)
		loss_func = torch.trace(net_end_x)
		...
		mse_f = pde1#+f2*pde2+f3*pde3

		# Combining the loss functions
		loss =mse_f + loss_func
		loss.backward()
		optimizer_x.step()
		optimizer_p.step()
		optimizer_u.step()
		with torch.no_grad():
			if epoch % 1000 == 0 and epoch > 0:
				print(
                    f"Epoch:{epoch} mse_init:{mse_init.item():.6f}  "
                    f"pde1:{pde1.item():.6f}  Trace:{loss_func.item():.6f}"
                )
		writer.add_scalar("Loss PDE 1", pde1, epoch)
		writer.add_scalar("Loss Functional", loss_func, epoch)
