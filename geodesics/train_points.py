...
# def pde_bc(
#     t_terminal, net_x, net_p, pf, boundary_constraint, manifold_constraint, loss_fn
# ):
...
#     all_zero = torch.zeros(1, device=device)
#     all_zero3 = torch.zeros((1, 3), device=device)
#     dx_dt = torch.cat(
#         [
#             torch.autograd.grad(
#                 x_tf,
...
#     g = energy_function(dx_dt)
#     dg_xdot = torch.autograd.grad(g.sum(), dx_dt, create_graph=True)[0]
#     loss1 = loss_fn(s, all_zero)
#     loss2 = loss_fn(pf * ds_dx + dg_xdot, all_zero3)
#     return loss1, loss2
...
if __name__ == "__main__":
    ...
    if manifold == "torus":
        ...
        x_init = np.array([[0, 1.5, 1.0]], dtype=np.float32)
        x_end = np.array([[0, 1.5, 1.0]], dtype=np.float32)
    elif manifold == "sphere":
        manifold_constraint = sphere_constraint
        x_init = np.array([[0, 1.0, 0.0]], dtype=np.float32)
        x_end = np.array([[0, 1.0, 0.0]], dtype=np.float32)
    ...
    # ReLU tanh SiLU
    net_x = sineNN(1, 3, activation_f,x_init,x_end).to(device)
...
# example command
# $ python train.py --iterations=50000 --load_model_x="torus/1_512x6_tanh/pretrained_x.pth" --load_model_p "torus/1_512x6_tanh/pretrained_p.pth" --folder_name="folder1"
